...
const TinyCLR_Api_Info* STM32F7_Can_GetApi() {
    for (int i = 0; i < TOTAL_CAN_CONTROLLERS; i++) {
        ...
        canProvider[i]->Reset = &STM32F7_Can_Reset;
...
TinyCLR_Result STM32F7_Can_SetBitTiming(const TinyCLR_Can_Provider* self, int32_t propagation, int32_t phase1, int32_t phase2, int32_t baudratePrescaler, int32_t synchronizationJumpWidth, int8_t useMultiBitSampling) {
    ...
    if (canController[channel].txMessage == nullptr)
        canController[channel].txMessage = (STM32F7_Can_TxMessage*)memoryProvider->Allocate(memoryProvider, canController[channel].can_txBufferSize * sizeof(STM32F7_Can_TxMessage));
    ...
    if (canController[channel].rxMessage == nullptr)
        canController[channel].rxMessage = (STM32F7_Can_RxMessage*)memoryProvider->Allocate(memoryProvider, sizeof(STM32F7_Can_RxMessage));
