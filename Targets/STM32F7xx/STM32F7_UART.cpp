...
#ifdef UART9
	if (TOTAL_UART_CONTROLLERS > 8) g_STM32F7_Uart_Ports[8] = UART9;
#endif
#endif
#endif
...
void STM32F7_Uart_Interrupt8(void* param) {
	...
	if (sr & (USART_ISR_RXNE | USART_ISR_ORE))
		STM32F7_Uart_IrqRx(8);

	if (sr & USART_ISR_TXE)
		STM32F7_Uart_IrqTx(8);
}
#endif
#endif
#endif
...
TinyCLR_Result STM32F7_Uart_SetActiveSettings(const TinyCLR_Uart_Provider* self, uint32_t baudRate, uint32_t dataBits, TinyCLR_Uart_Parity parity, TinyCLR_Uart_StopBitCount stopBits, TinyCLR_Uart_Handshake handshaking) {
	...
	uint16_t div = (uint16_t)( (clk) / baudRate ); // rounded - THIS WORK for OVER8 = 16
	g_UartController[portNum].portPtr->BRR = div;

	// control
	uint16_t ctrl_cr1 = USART_CR1_TE | USART_CR1_RE; // | USART_CR1_OVER8;
