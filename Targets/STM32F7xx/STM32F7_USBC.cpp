...
#if defined(USE_OTG_HS) || defined(OTG_USE_HS_ULPI)
#define STM32F7_USB_FS_MAX_EP   6	// OTG HS supports 6 endpoints
#else
...
#define STM32F7_USB_FS_TRDT			0x0F
#define STM32F7_USB_HS_TRDT			0x0A
...
bool STM32F7_UsbClient_ProtectUlpiPins(int controller, bool On);
...
static const int TOTAL_USB_CONTROLLERS = 1;
#else
...
bool STM32F7_UsbClient_Initialize(int controller) {
	...
	OTG->DCFG |= USB_OTG_DCFG_DSPD_0 | USB_OTG_DCFG_DSPD_1; // full speed
#endif // OTG_USE_HS_ULPI

	STM32F7_Time_DelayNoInterrupt(nullptr, 25000); // asure host recognizes reattach
	...
	//STM32F7_Time_DelayNoInterrupt(nullptr, 1000);

	OTG->GAHBCFG = 0;
	OTG->PCGCCTL = 0;
	//OTG->GCCFG |= USB_OTG_GCCFG_VBDEN; // VBUS detection

	if (STM32F7_USB_USE_VB_PIN(Controller))
	{
		OTG->GCCFG |= USB_OTG_GCCFG_VBDEN;         // enable vbus sense
	}
	if (!STM32F7_USB_USE_ID_PIN(Controller)) { // no ID pin
		OTG->GOTGCTL |= USB_OTG_GOTGCTL_BVALOEN    // B-peripheral session valid override enable
			| USB_OTG_GOTGCTL_BVALOVAL;  // B-peripheral session valid override value
	}
...
#endif
#ifdef OTG_USE_HS
	STM32F7_InterruptInternal_Activate(OTG_HS_IRQn, (uint32_t*)&STM32F7_UsbClient_FullspeedInterrupt, 0);
	STM32F7_InterruptInternal_Activate(OTG_HS_WKUP_IRQn, (uint32_t*)&STM32F7_UsbClient_FullspeedInterrupt, 0);
#else
	...
	OTG->DIEPMSK = 0;                 // disable Tx FIFO empty interrupts
	OTG->DIEPEMPMSK = 0;
	//OTG->DAINT = 0xFFFFFFFF;
	//OTG->GINTMSK = USB_OTG_GINTMSK_USBRST | USB_OTG_GINTMSK_WUIM | USB_OTG_GINTMSK_USBSUSPM;    
	OTG->GINTMSK = USB_OTG_GINTMSK_USBRST; // enable reset only
	OTG->GAHBCFG |= USB_OTG_GAHBCFG_GINT; // gloabl interrupt enable
...
#ifdef OTG_USE_HS_ULPI
	//RCC->AHB1ENR &= ~RCC_AHB1ENR_OTGHSULPIEN;
	STM32F7_UsbClient_ProtectUlpiPins(controller, false);

	for (uint8_t i = 0; i < TOTAL_ULPI_PINS; i++) {
...
int UsbClient_Driver::Write(int controller, int usbPipe, const char* Data, size_t size) {
	...
	if (nullptr == State
		|| usbPipe >= STM32F7_USB_QUEUE_SIZE
		|| Data == nullptr
